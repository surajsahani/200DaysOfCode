...
class Result {
    public static void main(String[] args) throws Exception{
        ...
        //Initialize Storage 2 d arraylist
        ...
        for(int a=xnum-1;a>=0;a--){
            ...
            for(int b=ynum-1 ;b>=0;b--){
                int j=y[b];
                for(int cell=0;cell<newprison.get(j).size();cell++){
                    newprison.get(j).set(cell, newprison.get(j).get(cell)   +newprison.get(j-1).get(cell));
                }  
                newprison.remove(j-1);
            }
            int max=1;
